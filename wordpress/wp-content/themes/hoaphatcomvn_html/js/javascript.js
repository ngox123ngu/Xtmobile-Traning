...
$(document).ready(function(){
  ...
  //check hoa don GHN
  var min = "-200px",max = "33px";
  $("#Tracking").click(function() {
    if($(".box_tracking_order").css("right") == min){// is it left?
      $(".box_tracking_order").animate({ right: max }); // move left
    }
    else{
      $(".box_tracking_order").animate({ right: min }); // move right
    }
  });
...
function doPostBDS(ref) {
  if(mem_id==0){
    jAlert('Vui lĂ²ng Ä‘Äƒng nháº­p thĂ nh viĂªn','BĂ¡o lá»—i',function() {popupLogin(ROOT+"member/dang-tin.html");});
  }else{
    window.location=ref;
  }
}
...
function doLogout()
{
  ...
  jConfirm('Báº¡n cĂ³ muá»‘n thoĂ¡t khá»i há»‡ thá»‘ng khĂ´ng ?', 'XĂ¡c nháº­n', function(r) {
    if (r){
...
{
    if($(".header-showrooms").hasClass("active"))
        $(".header-showrooms").removeClass("active");
    else
        $(".header-showrooms").addClass("active");
}